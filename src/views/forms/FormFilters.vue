<template>
  <div class="d-flex align-center mb-6" style="gap: 16px">
    ...
    <v-text-field
      v-model="search"
      :placeholder="`Buscar por Nombre, Folio o Campo`"
    ...
    <v-dialog v-model="dialog" max-width="420">
      <v-card class="modal-padding" style="position: relative">
        ...
        <v-card-text class="pb-0">
          ...
          <v-select
            v-model="supervisorRoleId"
            :items="supervisorRoleOptions"
            label="Supervisor"
            clearable
            hide-details
            variant="outlined"
          ...
          <v-select
            v-model="userRoleIds"
            :items="auditorRoleOptions"
            label="Roles de Auditor"
            multiple
            clearable
            hide-details
            variant="outlined"
          ...
          >
            <template #activator="{ props }">
              <v-text-field
                :model-value="formatDateOnly(createdAt)"
                ...
                @click:clear="createdAt = null"
              />
            </template>
            <v-date-picker v-model="createdAt" @update:modelValue="menuCreatedExact = false" />
          </v-menu>
        </v-card-text>
...
// Opciones simuladas, reemplaza por tus datos reales
const businessUnitOptions = [
  { title: 'Ventas', value: 1 },
  { title: 'Operaciones', value: 2 }
];
const supervisorRoleOptions = [
  { title: 'Supervisor A', value: 10 },
  { title: 'Supervisor B', value: 11 }
];
const auditorRoleOptions = [
  { title: 'Auditor X', value: 100 },
  { title: 'Auditor Y', value: 101 }
];
...
const supervisorRoleId = ref(null);
const userRoleIds = ref([]);
const createdAt = ref(null);
const menuCreatedExact = ref(false);
...
const hasActiveFilters = computed(
  () =>
    ...
    !!supervisorRoleId.value ||
    (userRoleIds.value && userRoleIds.value.length > 0) ||
    !!createdAt.value
);
...
function applyFilters() {
  emit('filter', {
    status: status.value,
    business_unit_id: businessUnitId.value,
    supervisor_role_id: supervisorRoleId.value,
    user_role_ids: userRoleIds.value,
    created_at: formatDateOnly(createdAt.value)
  });
  dialog.value = false;
}
