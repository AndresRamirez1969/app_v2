...
function filterResponses(responses, search) {
  ...
  return responses.filter((r) => {
    ...
    const addrRaw = props.getGeoAddress(r.value);
...
function parseCoords(value) {
  ...
  if (typeof value === 'object') {
    const latRaw = value.lat ?? value.latitude ?? value.latitud ?? value.coords?.lat ?? value.coords?.latitude ?? null;
    const lngRaw = value.lng ?? value.longitude ?? value.longitud ?? value.coords?.lng ?? value.coords?.longitude ?? null;
  ...
  return responses
    .map((r) => {
      ...
      return {
        ...
        label: r.folio || r.id || '-',
        user: r.user,
        address: props.getGeoAddress(r.value) || '-'
      };
    })
...
const filteredResponses = computed(() => filterResponses(props.fieldObj.responses || [], search.value));
        ...
        <tbody>
          <template v-for="(resp, i) in paginatedResponses" :key="i">
            <tr class="response-row record-row">
              ...
              <td class="response-value-cell">
                <a
                  v-if="props.getGeoAddress(resp.value) !== '-'"
                  :href="`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(props.getGeoAddress(resp.value))}`"
                  target="_blank"
                  rel="noopener noreferrer"
                  style="color: #1976d2; text-decoration: underline"
                >
                  {{ props.getGeoAddress(resp.value) }}
                </a>
                <span v-else>{{ props.getGeoAddress(resp.value) }}</span>
              </td>
              <td class="response-value-cell">
          ...
          <tr v-if="filteredResponses.length === 0">
            <td :colspan="hasScore ? 5 : 4" class="text-medium-emphasis">No hay registros de geolocalización.</td>
          </tr>
        </tbody>
      </v-table>
