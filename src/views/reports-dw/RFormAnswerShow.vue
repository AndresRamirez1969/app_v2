...
function _asUrl(u) {
  return typeof u === 'object' ? u?.url || u?.path || u?.file_path || u?.storage_path || '' : u;
}
...
async function fetchBinaryForUrl(originalUrl) {
  ...
  if (typeof url === 'string' && url.includes('amazonaws.com')) {
    ...
    // --- CORREGIDO: NO encodeURIComponent ---
...
/* ================== Navegación & formato ================== */
const goToIndex = () => router.push({ name: 'Report Answers DW', params: { formId } });
const formatScope = (s) =>
  ({ organization: 'Organización', business: 'Empresa', business_unit: 'Ubicación', business_unit_group: 'Grupo' })[s] || s || '—';
...
const orderedFieldsAndGroups = computed(() => {
  ...
  const fields = [...(formData.value.fields || [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  const groups = [...(formData.value.field_groups || [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  const groupMap = {};
  groups.forEach((g) => (groupMap[g.id] = { ...g, fields: [] }));
  fields.forEach((f) => {
...
/* ================== GetAnswer central ================== */
const getAnswer = (fieldId, fieldType, asArray = false, evidence = false) => {
  const resp = fieldResponses.value?.find((r) => r.form_field_id === fieldId || r.field_id === fieldId);
...
const downloadImagesZip = async (images, label = 'imagenes') => {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    try {
      ...
      console.log('Archivo:', imgUrl, 'Bin:', bin); // <-- Agrega este log
...
const downloadPdf = async () => {
  ...
  try {
    const res = await axios.get(`/reports/${reportId}/download-pdf`, { responseType: 'arraybuffer' });
  ...
  <v-container fluid>
        ...
        <v-menu location="bottom end">
          ...
          <v-list class="custom-dropdown elevation-1 rounded-lg" style="min-width: 200px">
            ...
            <template v-if="reportStatus === 'open'">
              ...
              <v-list-item @click="showCloseInfo = true">
                <template #prepend><v-icon :icon="mdiLock" size="18" /></template>
    ...
    <v-dialog v-model="showCloseInfo" max-width="520">
      <v-card class="close-info-card">
        <v-btn icon variant="text" class="close-info-x" @click="showCloseInfo = false" aria-label="Cerrar">
          <v-icon :icon="mdiClose" />
        </v-btn>
        <div class="close-info-icon">
          <v-icon :icon="mdiAlertCircleOutline" size="48" color="error" />
        </div>
        <v-card-title class="text-center pt-2"><span class="close-info-title">Cierre de Reporte</span></v-card-title>
        <v-card-text class="pt-2">
          <p class="mb-4 text-medium-emphasis text-center">El reporte pasará a cerrado y no podrá reabrirse.</p>
        </v-card-text>
        <v-card-actions class="px-6 pb-6">
          <v-btn variant="outlined" color="grey" @click="showCloseInfo = false">Cancelar</v-btn>
          <v-spacer />
          <v-btn variant="outlined" color="error" :loading="closingReport" @click="closeReport">Cerrar reporte</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-col cols="12">
        <div v-if="orderedFieldsAndGroups.length">
          <div v-for="(item, idx) in orderedFieldsAndGroups" :key="item.key" class="mb-4">
                  ...
                  <v-col>
                    ...
                    <v-card-text>
                      ...
                      <div class="mt-2 answer-row-bg" style="position: relative">
                        <template v-if="getAnswer(item.field.id, item.field.type)">
                          <!-- Imagen -->
                          <template v-if="item.field.type === 'image'">
                            <v-btn
                              v-if="filterImages(getAnswer(item.field.id, 'image', true)).length"
                              variant="text"
                              ...
                              @click="downloadImagesZip(getAnswer(item.field.id, 'image', true), item.field.label)"
                            >
                              <v-icon :icon="mdiFolderZipOutline" /><span class="custom-action-label">Descargar Zip</span>
                            </v-btn>
                            <div class="image-preview-row">
                              <img
                                v-for="(img, i) in filterImages(getAnswer(item.field.id, 'image', true))"
                                :key="i"
                                :src="typeof img === 'object' ? img.url : img"
                                alt="Evidencia"
                                style="width: 120px; height: 120px; object-fit: cover; border-radius: 8px; border: 1px solid #eee"
                                @click="openImageModal(i, filterImages(getAnswer(item.field.id, 'image', true)))"
                              />
                              <span v-if="!filterImages(getAnswer(item.field.id, 'image', true)).length">—</span>
                            </div>
                          </template>
                          ...
                          <template v-else-if="item.field.type === 'signature'">
                            <div class="signature-row" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: center">
                              <template v-for="(img, i) in filterImages(getAnswer(item.field.id, 'signature', true))" :key="i">
                                <img
                                  v-if="img"
                              ...
                              <span v-if="!filterImages(getAnswer(item.field.id, 'signature', true)).length">—</span>
                            </div>
                          </template>
                          ...
                          <template v-else-if="item.field.type === 'document'">
                            <div class="document-list">
                              <template v-if="filterDocuments(getAnswer(item.field.id, 'document', true)).length">
                                <div
                                  v-for="(doc, i) in filterDocuments(getAnswer(item.field.id, 'document', true))"
                                  :key="i"
                                  class="document-card"
                                >
                                  <v-icon
                          ...
                          <template v-else-if="isSemaforo(item.field)">
                            <div class="semaforo-chips-row semaforo-full-width">
                              <v-chip
                                ...
                                :style="chipStyleFilled(getAnswer(item.field.id, item.field.type), true)"
                                :ripple="false"
                              >
                                <span class="semaforo-chip-grid"
                                  ><span class="semaforo-check-left"></span
                                  ><span class="semaforo-text">{{ getAnswer(item.field.id, item.field.type) }}</span
                          ...
                          <template v-else-if="item.field.type === 'switch'">
                            <span>{{ getAnswer(item.field.id, item.field.type) }}</span>
                          </template>
                          <template v-else-if="item.field.type === 'url'">
                            <a
                              :href="getAnswer(item.field.id, item.field.type)"
                              target="_blank"
                              rel="noopener"
                              style="color: #1976d2; text-decoration: underline; word-break: break-all"
                            >
                              {{ getAnswer(item.field.id, item.field.type) }}
                            </a>
                          </template>
                          <template v-else-if="item.field.type === 'geolocation'">
                            <div v-if="getAnswer(item.field.id, item.field.type)">
                              <GoogleMap
                                v-if="getAnswer(item.field.id, 'geolocation')?.lat && getAnswer(item.field.id, 'geolocation')?.lng"
                                :points="[
                                  {
                                    lat: Number(getAnswer(item.field.id, 'geolocation')?.lat ?? 0),
                                    lng: Number(getAnswer(item.field.id, 'geolocation')?.lng ?? 0),
                                    label: getAnswer(item.field.id, 'geolocation')?.address || 'Ubicación'
                                  }
                                ]"
                                layer="points"
                                style="margin-bottom: 16px"
                              />
                              <div v-if="getAnswer(item.field.id, 'geolocation')?.address" style="margin-bottom: 16px; padding-top: 4px">
                                <strong>Dirección:</strong> {{ getAnswer(item.field.id, 'geolocation')?.address }}
                              </div>
                              <div>
                                <strong>Coordenadas:</strong>
                                {{ getAnswer(item.field.id, 'geolocation')?.lat }}, {{ getAnswer(item.field.id, 'geolocation')?.lng }}
                              </div>
                            </div>
                            <span v-else>—</span>
                          </template>
                      ...
                      >
                        <template
                          v-if="
                            filterImages(getAnswer(item.field.id, item.field.type, true, true)).length ||
                            filterDocuments(getAnswer(item.field.id, item.field.type, true, true)).length
                          "
                        >
                          <v-btn
                            v-if="filterImages(getAnswer(item.field.id, item.field.type, true, true)).length"
                            variant="text"
                            ...
                            @click="
                              downloadImagesZip(
                                filterImages(getAnswer(item.field.id, item.field.type, true, true)),
                          ...
                          <div v-if="filterImages(getAnswer(item.field.id, item.field.type, true, true)).length" class="image-preview-row">
                            <img
                              v-for="(img, i) in filterImages(getAnswer(item.field.id, item.field.type, true, true))"
                              :key="i"
                              :src="typeof img === 'object' ? img.url : img"
                              alt="Evidencia"
                              style="width: 120px; height: 120px; object-fit: cover; border-radius: 8px; border: 1px solid #eee"
                              @click="openImageModal(i, filterImages(getAnswer(item.field.id, item.field.type, true, true)))"
                            />
                          </div>

                          <div
                            v-if="filterDocuments(getAnswer(item.field.id, item.field.type, true, true)).length"
                            class="document-list"
                            style="margin-top: 16px"
                          >
                            <div
                              v-for="(doc, i) in filterDocuments(getAnswer(item.field.id, item.field.type, true, true))"
                              :key="i"
                              class="document-card"
                            >
                              <v-icon
            ...
            <!-- Si en el futuro usas grupos, el toggleGroup ya está listo -->
          </div>
        </div>
        <div v-else class="text-center pa-8"><p class="text-grey">No se pudo cargar el formulario o no tiene campos</p></div>
      </v-col>
    </v-row>
