...
async function fetchRole() {
  ...
  try {
    ...
    // --- INTEGRACIÓN: asegura que los permisos default estén seleccionados ---
    const perms = (data.permissions || []).map((perm) => perm.name);
    selectedPermissions.value = [
      ...defaultPermissions.filter((p) => perms.includes(p)),
      ...perms.filter((p) => !defaultPermissions.includes(p))
    ];
    // --- FIN INTEGRACIÓN ---
    if (isSuperadmin()) {
      ...
      <v-form class="mb-10">
        ...
        <v-row>
          <v-col cols="12">
            ...
            <v-text-field v-model="name" variant="outlined" color="primary" required class="mt-2 mb-4" :disabled="isProtectedRole" />
          </v-col>
        </v-row>
            ...
            <v-table class="rounded-lg elevation-1 uniform-table" style="width: 100%">
              ...
              <tbody>
                <tr
                  ...
                  :style="{ cursor: isProtectedRole || defaultPermissions.includes(perm.name) ? 'not-allowed' : 'pointer' }"
                  ...
                  <td class="text-center uniform-col" style="padding-left: 0; padding-right: 8px">
                    <div class="d-flex justify-center align-center" style="height: 100%">
                      <v-checkbox
                        ...
                        :ripple="false"
                        class="pa-0 ma-0"
                        :disabled="isProtectedRole || defaultPermissions.includes(perm.name)"
        ...
        <v-row v-else>
          <v-col cols="12" v-for="perm in permissionRows" :key="perm.id">
            <v-card
              ...
              elevation="1"
              :style="{
                cursor: isProtectedRole || defaultPermissions.includes(perm.name) ? 'not-allowed' : 'pointer',
                marginBottom: '20px',
                padding: '20px 16px'
              }"
            >
