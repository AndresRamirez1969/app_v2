...
const goToEdit = () => {
  if (userData.value?.id) {
    router.push({ path: `/usuarios-dw/${userData.value.id}/edit` });
  }
};

const goToIndex = () => {
  router.push('/usuarios-dw');
};

const goToOrganizationShow = (org) => router.push({ path: `/organizaciones-dw/${org.id}` });
const goToBusinessShow = (bus) => router.push({ path: `/negocios-dw/${bus.id}` });
const goToBusinessUnitShow = (unit) => router.push({ path: `/ubicaciones-dw/${unit.id}` });

const goToOrganizationEdit = (org) => router.push({ path: `/organizaciones-dw/${org.id}/edit` });
const goToBusinessEdit = (bus) => router.push({ path: `/negocios-dw/${bus.id}/edit` });
                          ...
                          <router-link
                            ...
                            :to="
                              item.tipo === 'Organización'
                                ? `/organizaciones-dw/${item.id}`
                                : item.tipo === 'Empresa'
                                  ? `/negocios-dw/${item.id}`
                                  : `/ubicaciones-dw/${item.id}`
                            "
