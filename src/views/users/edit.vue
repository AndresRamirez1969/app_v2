...
const canEditRoleAndScope = computed(() => {
  ...
  if (isAdmin.value) {
    return (
      form.organization_id === userOrgId.value || scopeOrgId.value === userOrgId.value
    );
  }
  if (isSponsor.value) {
    return (
      form.business_id === userBusinessId.value ||
      scopeBusinessId.value === userBusinessId.value
    );
  }
  return false;
});
  ...
  try {
    ...
    if (isSuperadmin.value) {
      ...
      if (
        ...
        String(user.organization_id) === String(userOrgId.value)
      ) {
        allowEdit = true;
      }
      ...
      if (
        ...
        (user.business_unit_id &&
          businessUnitOptions.value.some(
            (unit) =>
              String(unit.id) === String(user.business_unit_id) &&
              String(unit.business_id) === String(userBusinessId.value)
          ))
      ) {
        allowEdit = true;
      }
    ...
    if (user.organization_id && !user.business_id && !user.business_unit_id) {
      ...
      await fetchBusinessUnits("", {
        organization_id: scopeOrgId.value,
        business_id: scopeBusinessId.value,
      });
      form.organization_id = null;
      form.business_id = null;
    } else {
    ...
    if (isSuperadmin.value && user.organization_id) {
      ...
      if (form.role && !orgRoleOptions.value.find((r) => r.name === form.role))
        form.role = null;
    }
  } catch (err) {
    canEdit.value = false;
...
const validate = async () => {
  ...
  try {
    ...
    // Enviar los campos de alcance tal cual están
    formData.append("organization_id", form.organization_id ?? "");
    formData.append("business_id", form.business_id ?? "");
    formData.append("business_unit_id", form.business_unit_id ?? "");

    const hasContactData = Object.values(form.contact).some(
      (val) => val && val.trim() !== ""
    );
    if (hasContactData) {
