...
const goToEdit = () => {
  if (group.value?.id) {
    router.push({ path: `/grupos-dw/${group.value.id}/edit` });
  }
};

const goToIndex = () => {
  router.push('/grupos-dw');
};
      ...
      <v-col cols="12">
        ...
        <v-table class="rounded-lg elevation-1 fixed-table d-none d-md-table" style="width: 100%">
          ...
          <tbody>
            ...
            >
              <td class="folio-cell">
                <router-link :to="`/ubicaciones-dw/${unit.id}`" style="text-decoration: underline; color: #1976d2 !important" @click.stop>
              ...
              <v-col cols="8">
                <div class="d-flex align-center mb-1" style="justify-content: space-between">
                  <div class="text-caption" style="margin-right: 8px">
                    <router-link :to="`/ubicaciones-dw/${unit.id}`" @click.stop style="text-decoration: underline">
                      {{ unit.folio }}
                    </router-link>
                  </div>
