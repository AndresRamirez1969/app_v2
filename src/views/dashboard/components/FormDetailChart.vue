                    ...
                    <div v-for="field in geolocationFields" :key="field.id" class="mb-6">
                      <v-card variant="outlined">
                        <v-card-title class="text-subtitle-1">
                          {{ field.label }}
                          <v-chip size="small" color="info" class="ml-2"> {{ geolocationResponses(field.id).length }} ubicaciones </v-chip>
                        </v-card-title>
                        <v-card-text>
                ...
                >
                  <v-card variant="outlined" class="mb-4">
                    <v-card-text>
                      <div class="d-flex justify-space-between align-center mb-2">
                        <div>
                          <v-chip size="small" color="info" class="mb-2"> {{ numberStat.fieldLabel }} </v-chip>
  ...
  return fields.map((field) => {
    ...
    userResponses.forEach((userResponse) => {
      ...
      fieldResponses.forEach((fieldResponse) => {
        const fieldIdMatch =
          fieldResponse.field_id === field.id ||
    ...
    const average = numericValues.length > 0 ? numericValues.reduce((sum, value) => sum + value, 0) / numericValues.length : 0;
...
const getChartOptionsForField = (fieldStat) => {
  ...
  return {
    ...
    dataLabels: {
      enabled: true,
      offsetY: -20,
      style: { fontSize: '12px', colors: [dark ? '#fff' : '#304758'] }
    },
    xaxis: {
