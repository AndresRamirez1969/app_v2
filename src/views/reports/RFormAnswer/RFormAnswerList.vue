...
const fetchAnswers = async () => {
  ...
  try {
    ...
    const responses = Array.isArray(data.responses)
      ? data.responses
      : data.responses?.data || data.responses || [];
    ...
    items.value = responses.flatMap((resp) => {
      ...
      const userName =
        ...
        resp.user?.name ||
        resp.user_name ||
        (resp.response?.user && typeof resp.response.user === "string"
          ? resp.response.user
          : undefined) ||
        "—";
      ...
      return resp.reports.map((report) => ({
        ...
        ponderacion:
          hasRating.value &&
          report.ponderacion !== null &&
          report.ponderacion !== undefined &&
          Number(report.ponderacion) !== 0
            ? report.ponderacion
            : null,
        report,
        raw: { ...resp, report },
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                >
                  <a
                    :href="`/organizaciones/${formData.organization.id}`"
                ...
                <template
                  v-else-if="
                    formData?.assignment_scope === 'business' && formData?.business
                  "
                >
                  <a
                    :href="`/negocios/${formData.business.id}`"
                    style="color: #1976d2; text-decoration: underline"
                  >
                ...
                <template
                  v-else-if="
                    formData?.assignment_scope === 'business_unit' &&
                    formData?.business_unit
                  "
                >
                  <a
                    :href="`/ubicaciones/${formData.business_unit.id}`"
                    style="color: #1976d2; text-decoration: underline"
                  >
                ...
                <template
                  v-else-if="
                    formData?.assignment_scope === 'business_unit_group' &&
                    formData?.business_unit_group
                  "
                >
                  <a
                    :href="`/grupos/${formData.business_unit_group.id}`"
                    style="color: #1976d2; text-decoration: underline"
                  >
            ...
            <tr>
              ...
              <td>
                <span v-if="formData?.created_by">{{
                  formData.created_by.name || formData.created_by.email
                }}</span>
                <span v-else>No disponible</span>
              </td>
            </tr>
