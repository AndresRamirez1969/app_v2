...
const editOrg = async () => {
    try {
        ...
        await axiosInstance.put(`/businesses/${props.organizationId}`, cleanedForm)
        closeDialog()
...
<template>
    <v-dialog v-model="props.dialog" max-width="700px">
        <v-card>
      <v-card-title>
        <span v-if="activeForm === 'edit'">Editar Negocio</span>
        <span v-else-if="activeForm === 'contact'">Agregar Contacto</span>
        <span v-else-if="activeForm === 'business'">Agregar Unidades</span>
      </v-card-title>
      <v-card-actions>
        <v-btn 
        rounded="xl" 
